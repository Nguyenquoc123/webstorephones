...
import {
  ...
  fetchDetelePhienBan,
  fetchGetDSDienThoai,
  fetchGetDSDienThoaiPhanTrang,
  fetchGetDSPhienBan,
  fetchGetDSPhienBanPhanTrang,
...
function DienThoai() {
  ...
  useEffect(() => {
    console.log("run==================");
    if (menubar === 3) loadDSDienThoaiPhanTrang();
    else if (menubar === 4) loadDSPhienBanPhanTrang();
  ...
  const loadDSPhienBanPhanTrang = async () => {
    ...
    if (response.code === 200) {
      ...
      setDataPage({
        ...dataPage,
        totalPage: response.result.totalPages,
