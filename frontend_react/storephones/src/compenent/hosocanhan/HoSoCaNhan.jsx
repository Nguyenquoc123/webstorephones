...
const HoSoCaNhan = () => {
  ...
  const showGioiTinh = (value) => {
    if (!value) return '';
    if (1 === Number.parseInt(value))
      return "Nam"
    else if (2 === Number.parseInt(value))
      return "Nữ"
    return "Khác"
  }
  ...
  const data1 = [
    ...
    { label: "Giới tính", name: 'gioiTinh', value: showGioiTinh(info?.gioiTinh), editable: true },
    { label: "Ngày sinh", name: 'ngaySinh', value: convertDate(info?.ngaySinh), editable: true },
  ]
  const title2 = "Thông tin liên hệ";
  const data2 = [
    { label: "SDT", name: 'soDienThoai', value: info?.soDienThoai, editable: true },
  ...
  const data3 = [
    { label: "Tên đăng nhập", name: 'username', value: info?.username, editable: true },
    { label: "Mật khẩu", value: "*************", editable: false },
    { label: "Trạng thái TK", value: "Hoạt động", editable: false },
    { label: "Ngày tạo TK", value: convertDate(info?.ngayDangKy), editable: false },
  ]
  const title4 = "Thông tin giao dịch";
  const data4 = [
    { label: "Tổng số đơn hàng", value: info?.soDonHang, editable: false },
    { label: "Lần mua hàng gần nhất", value: convertDate(info?.ngayDatHangGanNhat), editable: false },
    { label: "Tổng giá trị mua", value: info?.tongTien.toLocaleString('vi-VN') + " đ", editable: false },
    ...
    <div>
      <div className="HSCN-profile-info">
        <label htmlFor="avatar-img">
          <img
            src={avatarImg && avatarImg instanceof File ? URL.createObjectURL(avatarImg) : (avatarImg ? avatarImg : "/images/anh-dai-dien-mac-dinh-18.jpg")}
            alt="avatar"
            className="avatar"
      ...
      <div className="btn-active-info">
        <button class="HSCN-btn_xac_nhan"
          onClick={updateHoSo}
        >Xác nhận</button>
      </div>
