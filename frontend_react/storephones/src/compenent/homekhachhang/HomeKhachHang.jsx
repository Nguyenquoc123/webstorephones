...
import {
  fetchGetDSPhienBan,
  fetchGetDSPhienBanAndKhuyenMai,
...
function HomeKhachHang() {
    ...
    if (key === "hang") {
      if (searchAndFilter.hang.includes(value)) {
        ...
        setSearchAndFilter((searchAndFilter) => ({
          ...searchAndFilter,
          [key]: [...searchAndFilter.hang, value],
        }));
      }
    }
    if (key === "boNho") {
      if (searchAndFilter.boNho.includes(value)) {
        ...
        setSearchAndFilter((searchAndFilter) => ({
          ...searchAndFilter,
          [key]: [...searchAndFilter.boNho, value],
        }));
      }
    }
    if (key === "gia") {
  ...
  const clickSearchAndFilter = async () => {
    ...
    let filter_ = false;
    if (
      searchAndFilter.hang.length > 0 ||
      searchAndFilter.boNho.length > 0 ||
      searchAndFilter.gia !== ""
    ) {
      filter_ = true;
    }
  ...
  return (
    <>
      <MenuKhachHang search={searchValue} />
      <ShowDSDienThoai
